...
//			Debug.log(String.format("rid.getPageId().pageNumber() = %d, pid.pageNumber() = %d, rid.getPageId().getTableId() = %d, pid.getTableId() = %d", rid.getPageId().pageNumber(), pid.pageNumber(), rid.getPageId().getTableId(), pid.getTableId()));
...
//			System.exit(1);
	...
	/**
	 * @return a stream containing all non-empty tuples
	 */
	...
	/**
	 * @return a stream containing all non-empty tuples in reverse order
	 */
	public Stream<Tuple> reversedStream() {
		return IntStream.range(0, tuples.length)
				.filter(i -> isSlotUsed(tuples.length - i - 1))
				.mapToObj(i -> tuples[tuples.length - i - 1]);
	}
}
